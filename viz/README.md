...
```bash
make viz-check                 # 40 random cases, ~4s
make viz-check VIZ_CHECK_N=200
```
...
Big enough for a real diagonal wavefront (a 2×2 barely has one), small enough
that every PE's weight and partial sum stay readable. `VIZ_ROWS`/`VIZ_COLS`/
`VIZ_MTILE` change the harness shape, but `viewer.html` and `model.mjs`'s
schedule assume 4×4/M_TILE=4.
